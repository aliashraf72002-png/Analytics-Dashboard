...
import { Canvas, ThreeEvent, useFrame, useThree, ThreeElements } from '@react-three/fiber'
import { shaderMaterial, useTrailTexture } from '@react-three/drei'
import { useTheme } from 'next-themes'
import * as THREE from 'three'

// Fix: Augment the global JSX namespace to include Three.js intrinsic elements.
// This resolves the errors where 'mesh', 'planeGeometry', and 'primitive' are not recognized as valid JSX elements.
declare global {
  namespace JSX {
...
function Scene({ theme }: { theme: string | undefined }) {
  ...
  const themeColors = useMemo(() => {
    switch (theme) {
      ...
      default:
        // Default to light to match SOP white background
        return {
          dotColor: '#e1e1e1',
          bgColor: '#FFFFFF',
          dotOpacity: 0.15
  ...
  const [trail, onMove] = useTrailTexture({
    ...
    maxAge: 400,
    interpolate: 1,
    ease: (x: number) => x < 0.5 ? (1 - Math.sqrt(1 - Math.pow(2 * x, 2))) / 2 : (Math.sqrt(1 - Math.pow(-2 * x + 2, 2)) + 1) / 2
  })
...
export const DotScreenShader = () => {
  const { theme } = useTheme() // Call hook in the same React root as the ThemeProvider
